...
Scope_Points = 10000  # total points to show in each channel in the scope
...
def logger(buffer_q, labels):
    ...
    with open('./osc/logging_stream_T.yaml', 'rt') as f:
...
if __name__ == '__main__':
    ...
    server = Server({'/Temperature': partial(make_document, buffer_q=graph_q, labels=y_labels)})
    server.start()
